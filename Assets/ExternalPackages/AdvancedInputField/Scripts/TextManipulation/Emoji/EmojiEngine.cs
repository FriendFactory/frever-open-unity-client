...
namespace AdvancedInputFieldPlugin
{
	...
	{
        private readonly HashSet<char> _emojiFirstChars = new HashSet<char>();
		...
		public void ConfigureEmojis(TMProEmojiAsset emojiAsset)
		{
			...
			for(int i = 0; i < length; i++)
			{
                ...
                _emojiFirstChars.Add(emoji.text[0]);
            }
		}
#endif
        ...
        public bool IsEmoji(char c) => _emojiFirstChars.Contains(c);

        public void ParseEmojiCharSequence(ref string text)
        {
            ...
            {
                if (TryFindNextEmojiInText(text, i, out var emojiData))
                {
                    stringBuilder.Append(emojiData.richText);
                    i += emojiData.text.Length;
                    continue;
                }

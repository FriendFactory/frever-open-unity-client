...
namespace AdvancedInputFieldPlugin
{
	public class RichTextProcessor
	{
		...
		public void SetupRichText(string richText)
		{
			...
			if(length == 1)
            {
                var region = parsedTextRegions[0];
                if (region.isSymbol)
                {
			...
			else //Text change
			{
				if(textEditFrame.selectionStartPosition == textEditFrame.selectionEndPosition && LastTextEditFrame.selectionStartPosition != LastTextEditFrame.selectionEndPosition) //Selection cleared
				{
					...
					else //Only clear
					{
                        ...
                        // space before selection on iOS is included in deletion even if it is not selected (see FREV-14044 for details)
                        var difference = LastTextEditFrame.selectionStartPosition - textEditFrame.selectionStartPosition;
                        if (difference > 0)
                        {
                            var text = LastTextEditFrame.text.Substring(LastTextEditFrame.selectionStartPosition - difference, difference);
                            textEditFrame.text = textEditFrame.text.Insert(textEditFrame.selectionStartPosition, text);
                            textEditFrame.selectionStartPosition += difference;
                            textEditFrame.selectionEndPosition += difference;
                        }
#endif

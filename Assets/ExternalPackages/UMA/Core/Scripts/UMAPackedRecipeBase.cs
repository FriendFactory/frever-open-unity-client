...
*/
		...
		public static List<UMADnaBase> UnPackDNA(List<UMAPackedRecipeBase.UMAPackedDna> DNA)
		{
			...
			foreach (UMAPackedDna packedDna in DNA)
			{
				...
				var dnaObj = UMADna.LoadInstance(dnaType, packedDna.packedDna);
				if (dnaObj.DNATypeHash != packedDna.dnaTypeHash)
				{
					dnaObj.DNATypeHash = packedDna.dnaTypeHash;
				}
				UnpackedDNA.Add(dnaObj);
			}
			return UnpackedDNA;
		}

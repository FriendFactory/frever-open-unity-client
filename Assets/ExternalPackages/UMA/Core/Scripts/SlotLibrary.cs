...
namespace UMA
{
	...
	{
		[SerializeField]
		protected SlotDataAsset[] slotElementList = Array.Empty<SlotDataAsset>();
		[NonSerialized]
	...
	#pragma warning restore 618
		...
		{
			ValidateDictionary();
            if (!slotDictionary.TryGetValue(nameHash, out var source))
			{
				return null;
			}
	...
	#pragma warning restore 618
        ...
        {
            foreach (var slot in slotElementList)
            {
                if (slot == null) continue;
            ...
            slotElementList = Array.Empty<SlotDataAsset>();
            slotDictionary.Clear();
        }
        ...
        public override void ReleaseLibraryPartially(PartialLibraryUnloadingArgs args)
        {
            ...
            var slotsToUnload = slotElementList.Where(x => !assetsToKeep.ContainsIgnoreCamelCase(x.slotName)).ToArray();
            slotElementList = slotElementList.Where(x => !slotsToUnload.Contains(x)).ToArray();

            foreach (var slot in slotsToUnload)
            {
                slotDictionary.Remove(slot.nameHash);
                if (slot == null) continue;

			...
			for (int textureType = slotData.asset.material.channels.Length - 1; textureType >= 0; textureType--)
			{
			   switch (slotData.asset.material.channels[textureType].channelType)
			   {
				  ...
				  case UMAMaterial.ChannelType.NormalMap:
				  {
					 ...
					 if (downSample != 0)
					 {
						int newW = Mathf.Max(8, width >> downSample) ;
						int newH = Mathf.Max(8, height >> downSample);
                     ...
                     var destWidth = destinationTexture.width;
                     var destHeight = destinationTexture.height;
                     var destText = destinationTexture;
					 ...
					 {
						 // This path converts already the render texture to a normal texture, so umaGenerator.convertRenderTexture is implicit in this case
						 int mipmapCount = Mathf.Min(destinationTexture.mipmapCount, PVRTCComputeInstance.MaxPVRTCMipmapSupported(destinationTexture.width));
                         
						 // Only allow a certain number of compressions in parallel
						 while (!CheckReadyToCompress())
						 {
							 yield return null;
						 }
                         
						 PVRTCComputeManager.Compress(destText, mipmapCount, (NativeArray<byte> compressed) =>
                         {
                             // Create Replace texture with serialised data

                             var tempTexture = new Texture2D(destWidth, destHeight, TextureFormat.PVRTC_RGBA4, mipmapCount, true);
                             tempTexture.LoadRawTextureData(compressed);
                             tempTexture.Apply(false, true);		
							
                             // Apply texture settings
                             tempTexture.wrapMode = TextureWrapMode.Repeat;
                             tempTexture.anisoLevel = localSlotData.asset.material.AnisoLevel;
                             tempTexture.mipMapBias = localSlotData.asset.material.MipMapBias;
                             tempTexture.filterMode = localSlotData.asset.material.MatFilterMode;

                             // Assign the compressed texture
                             localResultingTextures[localTextureType] = tempTexture;
                             localAtlas.material.SetTexture(localSlotData.asset.material.channels[localTextureType].materialPropertyName, tempTexture);
							 
                             // Signal that a compression has finished
                             SignalCompressed();
                         });

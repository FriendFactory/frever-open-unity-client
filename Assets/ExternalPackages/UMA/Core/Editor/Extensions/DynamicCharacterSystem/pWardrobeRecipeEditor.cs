...
namespace UMA.Editors
{
	public partial class RecipeEditor
	{
		...
		protected virtual bool DrawWardrobeSlotsFields(Type TargetType, bool ShowHelp = false)
		{
			...
			FieldInfo RelevanceValueField = TargetType.GetField("RelevanceValue", BindingFlags.Public | BindingFlags.Instance);
			int relevanceValue = (int) RelevanceValueField.GetValue(target);
			...
			#region Relevance Value UI
			//relevanceValue UI
			int PreviousRelevance = relevanceValue;
			relevanceValue = EditorGUILayout.DelayedIntField("Relevance", relevanceValue);
			if (relevanceValue != PreviousRelevance)
			{
				RelevanceValueField.SetValue(target, relevanceValue);
				doUpdate = true;
			}
			if (ShowHelp)
			{
				EditorGUILayout.HelpBox("Relevance values allows for sorting wardrobe slots based on a given relevance. This is useful in scenarios where wardrobe slots are listed in eg. a character editor.", MessageType.Info);
			}
			#endregion

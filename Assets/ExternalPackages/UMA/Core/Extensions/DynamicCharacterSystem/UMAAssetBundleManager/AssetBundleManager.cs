	   ...
	   ///  any asset bundle without having to download it first. 
		...
		static protected bool LoadAssetBundleInternal(string assetBundleToFind, bool isLoadingAssetBundleIndex = false, bool useJsonIndex = false, string jsonIndexUrl = "")
		{
			...
			else
			{
                ...
                var encryptionService = EncryptionServiceProvider.GetEncryptionService(); //DWC some encryption here
                if (bundleBaseDownloadingURL.EndsWith(encryptionService.TargetExtension))
                {
                    const string filePrefix = "file://";
                    var assetBundlePath = bundleBaseDownloadingURL.StartsWith(filePrefix)
                        ? bundleBaseDownloadingURL.Substring(filePrefix.Length)
                        : bundleBaseDownloadingURL;
                    
                    UnityEngine.Debug.Log("DWC encryption path" + assetBundlePath);
                    m_InProgressOperations.Add(new AssetBundleLoadEncryptedOperation(assetBundleToFind /* + encryptedSuffix*/, assetBundlePath, encryptionService));
                }
                else
                {
#if UNITY_2017_2_OR_NEWER

...
namespace UMA.AssetBundles
{
	...
	public partial class AssetBundleIndex : ScriptableObject
	{
		...
		public partial class AssetBundleIndexList
		{
			...
			{
				var thisItem = new AssetBundleIndexItem();
		...
		{
			for (var i = 0; i < bundlesIndex.Count; i++)
			{
		...
		public string[] GetAllAssetBundleNames()
		{
			...
			foreach (var bundle in bundlesIndex)
			{
				assetBundleNames.Add(bundle.assetBundleName);
			}
		...
		{
			var assetFoundIn = new List<string>();
			for (var i = 0; i < bundlesIndex.Count; i++)
			{
				for (var ii = 0; ii < bundlesIndex[i].assetBundleAssets.Count; ii++)
				{
		...
		{
			var assetName = "";
			var foundInBundles = FindContainingAssetBundle(filename, type);
		...
		{
			var assetName = "";
			var foundInBundles = FindContainingAssetBundle(assetNameHash, type);
			if (foundInBundles.Length > 0)
			{
		...
		static string GetTypeWithoutAssembly(string fullType)
		{
			...
			var typeWithoutAssembly = typeParts[typeParts.Length - 1];

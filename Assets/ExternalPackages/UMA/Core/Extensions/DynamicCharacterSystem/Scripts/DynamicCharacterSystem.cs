...
namespace UMA.CharacterSystem
{
	public class DynamicCharacterSystem : DynamicCharacterSystemBase
	{
		...
		private void ClearCacheUMATextRecipes(List<UMATextRecipe> recipeList)
		{
			foreach (var entry in recipeList)
			{
				if (entry == null) continue;
				
				Resources.UnloadAsset(entry);
			}
			recipeList.Clear();
		}
		
		private void ClearCacheRaceRecipes()
		{
			foreach (var raceRecipes in Recipes)
			{
				foreach (var recipes in raceRecipes.Value)
				{
					ClearCacheUMATextRecipes(recipes.Value);
				}
			}
		}
		
		private void ClearCacheRecipeIndex()
		{
			foreach (var recipeIndexEntry in RecipeIndex)
			{
				if (recipeIndexEntry.Value.MeshHideAssets != null)
				{
					recipeIndexEntry.Value.MeshHideAssets.Clear();
				}

				recipeIndexEntry.Value.ResetCachedRecipe();
			}
		}
		
		public void ClearCacheRecipes()
		{
			ClearCacheRaceRecipes();
			ClearCacheRecipeIndex();
		}
		...
		private void GatherRecipeFiles(string filename = "", string bundleToGather = "")
		{
			...
			if (DynamicAssetLoader.Instance != null)
			{
                ...
                DynamicAssetLoader.Instance.AddAssets<UMATextRecipe>(ref assetBundlesUsedDict, dynamicallyAddFromResourcesNow, dynamicallyAddFromAssetBundles, downloadAssetsEnabledNow, assetBundleToGather, resourcesRecipesFolder, null, filename, AddRecipesFromAB); // Fenrir 09.12.19

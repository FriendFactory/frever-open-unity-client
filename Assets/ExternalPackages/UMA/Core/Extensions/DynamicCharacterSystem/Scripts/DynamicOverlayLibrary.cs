...
namespace UMA.CharacterSystem
{
    ...
    #pragma warning restore 618
        ...
        {
            base.ReleaseLibraryPartially(args);
            var allBundles = assetBundlesUsedDict.Keys;
            var bundlesToKeep = args.BundlesToKeep;
            var bundlesToUnload = allBundles.Where(x => !bundlesToKeep.ContainsIgnoreCamelCase(x)).ToArray();
            foreach (var bundleToUnload in bundlesToUnload)
            {
                assetBundlesUsedDict.Remove(bundleToUnload);
            }
        }
    }
}

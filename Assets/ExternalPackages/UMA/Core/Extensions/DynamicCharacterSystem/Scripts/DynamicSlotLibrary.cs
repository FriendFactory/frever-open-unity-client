...
namespace UMA.CharacterSystem
{
    public class DynamicSlotLibrary : SlotLibrary
    {
        ...
        public override void ReleaseLibraryPartially(PartialLibraryUnloadingArgs args)
        {
            ...
            var unloadedBundles = assetBundlesUsedDict.Keys.Where(x => !args.BundlesToKeep.ContainsIgnoreCamelCase(x)).ToArray();
            foreach (var unloadedBundle in unloadedBundles)
            {
                assetBundlesUsedDict.Remove(unloadedBundle);
            }
        }
    }
}
